...
df = pd.read_csv("z_test/studentScore.csv")
data = df['Math_score'].tolist()

#fig = ff.create_distplot([data],["Math Score"],show_hist= False)
#fig.show()

#calculating mean and standard deviation of population data
...
#df = pd.read_csv("z_test/data1.csv")
#data = df["Math_score"].tolist()
...
# finding the mean of the first data(STUDENTS WHO GOT TABLET WITH LEARNING MATERIAL) and plotting it on the plot.
#df = pd.read_csv("z_test/data2.csv")
#data = df["Math_score"].tolist()
...
df = pd.read_csv("z_test/data3.csv")
data = df["Math_score"].tolist()
